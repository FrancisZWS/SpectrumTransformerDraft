...
for i = 1:10
    dataIn = randi([0 M-1],nfft, nSym, nt);% Generate random data bits
    qamSig = qammod(dataIn, M, 'UnitAveragePower', true);
    modData = ofdmmod(qamSig,nfft,cplen);
    ...
    txfilterOut = txFilter( modData(:,1) ); %"RolloffFactor",0.5,
    figure(1)
    spectrum=fftshift(abs(fft(txfilterOut, nfft*fft_n)).^2);
